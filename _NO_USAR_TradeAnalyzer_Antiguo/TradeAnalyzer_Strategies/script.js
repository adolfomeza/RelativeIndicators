    ...
    const filterAttempt = document.getElementById('filter-attempt'); // v1.5: Attempt filter
    const resetFiltersBtn = document.getElementById('reset-filters-btn');
    
    // --- Persistence Logic ---
    // DEBUG_MODE: Set to true to disable localStorage saving (clean slate on each refresh)
    const DEBUG_MODE = true; // ← Change to false when done debugging
    ...
    function saveData() {
        if(DEBUG_MODE) return; // Skip saving in debug mode
    ...
    // Load on start (skip if DEBUG_MODE is on)
    if(!DEBUG_MODE) {
        ...
        console.log("DEBUG_MODE: Skipping localStorage load");
    }
    
    // v1.5: Auto-Load CSV Data from load_data.ps1 script
    try {
        if (window.AUTO_CSV_DATA && window.AUTO_CSV_DATA.length > 0) {
            console.log("Auto-Loading CSV Data:", window.AUTO_CSV_DATA.length, "trades");
            
            // Convert dates and ensure numbers
            const autoTrades = window.AUTO_CSV_DATA.map(t => {
                let d = new Date(t.entryTime);
                if (isNaN(d.getTime())) {
                    d = new Date(t.entryTime.split('.')[0]); 
                }
                
                let dExit = t.exitTime ? new Date(t.exitTime) : null;
                if (dExit && isNaN(dExit.getTime())) {
                ...
                return {
                    ...
                    mae: Number(t.mae) || 0,
                    mfe: Number(t.mfe) || 0,
                    attempt: parseInt(t.attempt) || 1,
                    riskReward: Number(t.riskReward) || 0
                };
            });
            
            handleAutoFiles(autoTrades);
            console.log("AUTO_CSV_DATA loaded successfully!");
        }
    } catch (err) {
        console.warn("Auto-Load CSV Error:", err.message);
    }
    ...
    function populateFilters(trades) {
         ...
         // v1.5: Populate Attempt filter dynamically
         if(filterAttempt) {
             const attempts = [...new Set(trades.map(t => t.attempt || 1))].sort((a,b) => a - b);
             const currentAttemptVal = filterAttempt.value;
             
             filterAttempt.innerHTML = '<option value="all">All</option>';
             
             attempts.forEach(att => {
                 const option = document.createElement('option');
                 option.value = att;
                 option.textContent = att;
                 filterAttempt.appendChild(option);
             });
             
             // Restore selection if valid
             if(attempts.includes(parseInt(currentAttemptVal))) {
                 filterAttempt.value = currentAttemptVal;
             }
         }
    }
    ...
    function applyFilters() {
        ...
        const attemptVal = filterAttempt ? filterAttempt.value : 'all'; // v1.5: Attempt filter
        ...
        const filtered = globalAllTrades.filter(t => {
            ...
            // v1.5: Attempt Filter (exact match)
            if(attemptVal !== 'all') {
                const att = t.attempt || 1;
                if (att.toString() !== attemptVal) return false;
            }
    ...
    [filterInstrument, filterDirection, filterDay, filterHour, filterTarget, filterComponent, filterAttempt].forEach(el => {
    ...
    if(resetFiltersBtn) {
        resetFiltersBtn.addEventListener('click', () => {
             ...
             if(filterAttempt) filterAttempt.value = 'all';
             applyFilters();
        });
    }
    
    // v1.5: Clear All Data button
    const clearAllBtn = document.getElementById('clear-all-btn');
    if(clearAllBtn) {
        clearAllBtn.addEventListener('click', () => {
            if(confirm('¿Borrar TODOS los datos?\n\nEsto limpiará:\n• localStorage\n• Datos en memoria\n• AUTO_CSV_DATA\n\nNota: Si quieres que NO se recarguen datos automáticamente, borra los CSVs de trace\\TradeAnalyzer\\ y corre load_data.ps1 de nuevo.')) {
                // Clear all data sources
                localStorage.clear();
                globalAllTrades = [];
                window.AUTO_CSV_DATA = []; // Clear auto-loaded data
                
                // Reset UI without reloading (reload would re-load auto_data.js!)
                uploadArea.classList.remove('hidden');
                dashboard.classList.add('hidden');
                
                // Clear filters
                filterInstrument.innerHTML = '<option value="all">All</option>';
                if(filterAttempt) filterAttempt.innerHTML = '<option value="all">All</option>';
                
                // Clear stats
                document.getElementById('stat-net-profit').textContent = '$0.00';
                ...
                document.getElementById('stat-avg-win-loss').textContent = '$0.00 / $0.00';
                ...
                alert('✅ Datos borrados. Para evitar que vuelvan a aparecer, borra los CSVs y regenera auto_data.js');
            }
        });
    }
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            try {
                ...
                const trade = {
                    ...
                    grossPnl: parseNum(cols[8]), // v1.5.1: Gross PnL (before commissions)
                    commission: cols[9] ? parseNum(cols[9]) : 0, // v1.5.1: Commission
                    pnl: cols[10] ? parseNum(cols[10]) : parseNum(cols[8]), // v1.5.1: NetPnL (fallback to gross if not present)
                    mae: cols[11] ? parseNum(cols[11]) : 0,
                    mfe: cols[12] ? parseNum(cols[12]) : 0,
                    setup: cols[13] || '',  // v1.5: Setup name (Europe Low, etc.)
                    attempt: cols[14] ? parseInt(cols[14]) : 1,  // v1.5: VWAP attempt number
                    riskReward: cols[15] ? parseNum(cols[15]) : 0  // v1.5: R:R ratio for distribution chart
                };
    ...
    function processData(trades) {
        ...
        const hourStats = {}; 
        const dayStats = {};
        const attemptStats = {}; // v1.5: PnL by attempt number  
        ...
        trades.forEach((trade, index) => {
            if (!isNaN(trade.pnl)) {
                ...
                if (!attemptStats[attempt]) attemptStats[attempt] = 0;
                attemptStats[attempt] += trade.pnl;
            }
        });
        ...
        renderAttemptChart('attemptChart', attemptStats); // v1.5: PnL by Attempt
        renderScatterChart('maeChart', trades, 'mae', 'pnl');
        renderScatterChart('mfeChart', trades, 'mfe', 'pnl');
        renderRRDistribution('rrDistChart', trades); // v1.5: R:R Distribution
    ...
    // --- Audit Stats (v1.5: Monte Carlo, T-Test, Sharpe, MAE/MFE) ---
    function calculateAuditStats(trades) {
        if (!trades || trades.length < 3) {
            console.log("Not enough trades for audit stats");
            return;
        }
        
        const pnls = trades.map(t => t.pnl).filter(p => !isNaN(p));
        if (pnls.length < 3) return;
        
        // --- T-Test ---
        try {
            const ttest = MathUtils.tTestInfo(pnls);
            const ttestEl = document.getElementById('audit-ttest');
            const ttestDescEl = document.getElementById('audit-ttest-desc');
            if (ttestEl) {
                ttestEl.textContent = ttest.t.toFixed(2);
                if (ttestDescEl) {
                    ttestDescEl.textContent = ttest.significant 
                        ? "✅ Edge Significativo (p<0.05)" 
                        : "⚠️ No Significativo";
                    ttestDescEl.className = 'stat-sub ' + (ttest.significant ? 'positive-val' : '');
                }
            }
        } catch (e) {
            console.warn("T-Test Error:", e);
        }
        ...
        try {
            ...
            const halfLen = Math.floor(pnls.length / 2);
            
            for (let i = 0; i < 1000; i++) {
                const shuffled = MathUtils.shuffle([...pnls]);
                const shuffledSum = shuffled.slice(0, halfLen).reduce((a, b) => a + b, 0);
                if (shuffledSum >= realSum) betterCount++;
            }
            
            const luckProb = (betterCount / 10).toFixed(1); // Convert to percentage
            const mcEl = document.getElementById('audit-montecarlo');
            const mcDescEl = document.getElementById('audit-montecarlo-desc');
            if (mcEl) {
                mcEl.textContent = luckProb + "%";
                if (mcDescEl) {
                    mcDescEl.textContent = parseFloat(luckProb) < 5 
                        ? "✅ Resultados NO son suerte" 
                        : "⚠️ Posible suerte";
                    mcDescEl.className = 'stat-sub ' + (parseFloat(luckProb) < 5 ? 'positive-val' : '');
                }
            }
        } catch (e) {
            console.warn("Monte Carlo Error:", e);
        }
        ...
        try {
            ...
            const sharpeEl = document.getElementById('audit-sharpe');
            if (sharpeEl) {
                sharpeEl.textContent = sharpe;
            }
        } catch (e) {
            console.warn("Sharpe Error:", e);
        }
        
        // --- MAE/MFE Averages ---
        try {
            const maes = trades.map(t => t.mae || 0).filter(v => !isNaN(v));
            const mfes = trades.map(t => t.mfe || 0).filter(v => !isNaN(v));
            
            const avgMAE = maes.length > 0 ? MathUtils.mean(maes) : 0;
            const avgMFE = mfes.length > 0 ? MathUtils.mean(mfes) : 0;
            
            const maeEl = document.getElementById('audit-mae');
            const mfeEl = document.getElementById('audit-mfe');
            if (maeEl) maeEl.textContent = formatCurrency(avgMAE);
            if (mfeEl) mfeEl.textContent = formatCurrency(avgMFE);
            
            // Exit Efficiency: How much of MFE did we capture?
            const totalPnL = pnls.reduce((a, b) => a + b, 0);
            const totalMFE = mfes.reduce((a, b) => a + b, 0);
            const efficiency = totalMFE > 0 ? (totalPnL / totalMFE * 100).toFixed(1) : "N/A";
            
            const effEl = document.getElementById('audit-efficiency');
            if (effEl) effEl.textContent = efficiency + "%";
        } catch (e) {
            console.warn("MAE/MFE Error:", e);
        }
        
        console.log("Audit Stats calculated for", trades.length, "trades");
    }
    ...
    // v1.5: R:R Distribution Chart (TP1 vs TP2) - DYNAMIC BUCKETS
    function renderRRDistribution(id, trades) {
        const ctx = document.getElementById(id);
        if (!ctx) return;
        
        if (charts[id]) charts[id].destroy();
        
        // Separate TP1 and TP2 trades
        const tp1Trades = trades.filter(t => (t.result || '').toUpperCase().includes('TP1'));
        const tp2Trades = trades.filter(t => (t.result || '').toUpperCase().includes('TP2'));
        
        // v1.5.2: DYNAMIC BUCKETS - Find min and max R:R from all trades
        const allRR = trades.map(t => t.riskReward || 0).filter(rr => !isNaN(rr));
        const minRR = allRR.length > 0 ? Math.floor(Math.min(...allRR) * 2) / 2 : -2; // Round down to 0.5
        const maxRR = allRR.length > 0 ? Math.ceil(Math.max(...allRR) * 2) / 2 : 3;   // Round up to 0.5
        
        // Generate buckets from minRR to maxRR in 0.5 increments
        const buckets = [];
        for (let r = minRR; r <= maxRR; r += 0.5) {
            buckets.push(Math.round(r * 10) / 10); // Avoid floating point issues
        }
        const bucketLabels = buckets.map(b => b.toString());
        
        // Count trades in each bucket (0.5 range per bucket)
        const countInBucket = (tradeList, bucketVal, isLast) => {
            return tradeList.filter(t => {
                const rr = t.riskReward || 0;
                if (isLast) return rr >= bucketVal; // Last bucket includes all >= value
                return rr >= bucketVal && rr < bucketVal + 0.5;
            }).length;
        };
        
        const tp1Data = buckets.map((b, i) => countInBucket(tp1Trades, b, i === buckets.length - 1));
        const tp2Data = buckets.map((b, i) => countInBucket(tp2Trades, b, i === buckets.length - 1));
        ...
        charts[id] = new Chart(ctx.getContext('2d'), {
            ...
            options: {
                ...
                scales: {
                    x: { 
                        title: { display: true, text: 'Risk-Reward Ratio (R)' },
                        grid: { color: '#334155' }
                    },
                    y: { 
    ...
    // v1.5: PnL by Attempt Number Chart
    function renderAttemptChart(id, attemptData) {
        const ctx = document.getElementById(id);
        if (!ctx) return;
        
        if (charts[id]) charts[id].destroy();
        
        // Sort attempts numerically
        const labels = Object.keys(attemptData).sort((a,b) => parseInt(a) - parseInt(b));
        const dataValues = labels.map(l => attemptData[l] || 0);
        const bgColors = dataValues.map(v => v >= 0 ? '#10b981' : '#ef4444');
        ...
        charts[id] = new Chart(ctx.getContext('2d'), {
            ...
            options: {
                ...
                scales: {
                    ...
                    y: { 
                        title: { display: true, text: 'Total PnL ($)' },
                        grid: { color: '#334155' }
                    }
                },
                plugins: {
